def delay_query(stop_id = None, trip_id = None):
    ...
    WITH LatestTripUpdate AS (
        SELECT
            ...
            ROW_NUMBER() OVER (PARTITION BY trip_updates.trip_id, stop_time_updates.stop_id, stop_times.arrival_time ORDER BY trip_updates.oid DESC) AS rn
        FROM
            stop_time_updates
        JOIN
    ...
    ORDER BY trip_id, planned_arrival_time;
    """