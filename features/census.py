...
class Query:
    ...
    def get_batched_queries(self):
        for i in range(0, len(self.tables), 30):
            yield f"https://api.census.gov/data/{self.census_data_source}?get={','.join([str(t) for t in self.tables[i:i+30]])}&for=block%20group:*&in=state:{str(self.state_code).zfill(2)}&in=county:{str(self.county_code)}&in=tract:{','.join([str(t) for t in self.tracts])}"

    def get(self):
        df = pd.DataFrame(columns=Query.GEO_FIELDS)
        
        for url in tqdm(list(self.get_batched_queries())):
            r = requests.get(url)
